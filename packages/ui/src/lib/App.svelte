<script lang="ts">
  ...
  const buffer = new MirrorBuffer<eventWithTime>({
    ...
    onChunk(chunk) {
      ...
      /**
       * TODO: Analyzee 28.04.2023.
       */
      if (!controlCurrent.matches('controlling') || !isIgnoredOnRemoteControl(data)) {
        replayer.addEvent(data);
      } else {
        // console.log('control event');
        // applyMirrorAction(replayer.getMirror(), payload as MirrorPayload);
      }
    }
  });
